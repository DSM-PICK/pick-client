      ...
      {recordDataArr.map(data => (
        <S.GradeWrapper key={data.title}>
          ...
          <S.RecordLine isGray={true}>
            <S.RecordText>{`이동 : ${data.move || 0}번`}</S.RecordText>
            <S.RecordText>{`외출 : ${data.outing || 0}번`}</S.RecordText>
          </S.RecordLine>
          <S.RecordLine>
            <S.RecordText>{`현체 : ${
              data.fieldExperience || 0
            }번`}</S.RecordText>
            <S.RecordText>{`귀가 : ${data.homeComing || 0}번`}</S.RecordText>
          </S.RecordLine>
          <S.RecordLine isGray={true}>
            <S.RecordText isHighlight={true}>{`무단 : ${
              data.truancy || 0
            }번`}</S.RecordText>
          </S.RecordLine>
        </S.GradeWrapper>
